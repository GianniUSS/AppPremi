...
def find_col(df: pd.DataFrame, candidates: list[list[str]], required: bool = True):
...
def parse_preparatori(file_path: str) -> pd.DataFrame:
    ...
    # funzione conversione YYYYMMDD -> YYYY-MM-DD
    def convert_data(val):
        ...
        if sval.isdigit() and len(sval) == 8:
            try:
                return datetime.datetime.strptime(sval, "%Y%m%d").date()
            except:
                return None
        try:
            return pd.to_datetime(sval, errors="coerce").date()
        except:
            return None

    sub[col_data] = sub[col_data].apply(convert_data)
    ...
    out = sub.groupby(["data", "codice_preparatore", "nome_preparatore"], as_index=False)["totale_colli"].sum()
    out["penalita"] = 0
    out["tipo_attivita"] = "PICKING"
    out["tipo"] = ""
    return out
...
def parse_ricevitori(file_path: str) -> pd.DataFrame:
    ...
    records = []
    for _, row in df.iterrows():
        codice = row[1]   # colonna B
        data_raw = row[8] # colonna I
        colli = row[15]   # colonna P
...
# ============== IMPORT PRINCIPALE ==============
def importa_excel(progress_var: IntVar, progress_bar: ttk.Progressbar, status_label: ttk.Label):
    try:
...
ttk.Button(
    root, text="Sfoglia...",
    command=lambda: file_entry.delete(0, "end") or file_entry.insert(
...
import_button = ttk.Button(root, text="Importa dati",
                           command=lambda: importa_excel(progress_var, progress_bar, status_label))
